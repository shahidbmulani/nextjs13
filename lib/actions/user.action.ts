...
export async function getUserInfo(params: GetUserByIdParams) {
    ...
    try {
        ...
        const criteria = [
            ...
            { type: 'QUESTION_COUNT' as BadgeCriteriaType, count: answerUpvotes?.totalUpvotes || 0 },
            { type: 'TOTAL_VIEWS' as BadgeCriteriaType, count: questionViews?.totalUpvotes || 0 },
        ]
...
export async function getUserAnswers(params: GetUserStatsParams) {
    ...
    try {
        ...
        const userAnswers = await Answer.find({ author: userId }).sort({ upvotes: -1 }).skip(skipAmount).limit(pageSize).populate('question', '_id title').populate('author', '_id clerkId name picture');
